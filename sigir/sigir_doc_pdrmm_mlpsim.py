...
class Sent_Posit_Drmm_Modeler(nn.Module):
    ...
    def emit_doc_cnn(self, doc_embeds, question_embeds, q_conv_res_trigram, q_weights, sim_oh):
        ...
        sim_insens = self.my_sim_noncontext(question_embeds, doc_embeds).squeeze(0)
        sim_sens = self.my_sim_contextual(q_conv_res_trigram, conv_res).squeeze(0)
    ...
    def emit_one(
        ...
        # HANDLE DOCS
        good_out = self.emit_doc_cnn(doc1_embeds, question_embeds, q_context, q_weights, oh_sim_1)
        #
    ...
    def forward(
        ...
        # HANDLE DOCS
        good_out = self.emit_doc_cnn(doc1_embeds, question_embeds, q_context, q_weights, oh_sim_1)
        bad_out = self.emit_doc_cnn(doc2_embeds, question_embeds, q_context, q_weights, oh_sim_2)
        #
        good_out_pp = torch.cat([good_out, doc_gaf], -1)

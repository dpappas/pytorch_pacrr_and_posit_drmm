...
def prep_data(quest, the_doc, pid, the_bm25, idf, max_idf):
    ...
    doc_embeds          = np.concatenate(all_bert_embeds, 0)
    ####
...
def train_data_step2(instances, docs, idf, max_idf):
...
class Sent_Posit_Drmm_Modeler(nn.Module):
    ...
    def emit_doc_cnn(self, doc_embeds, oh_sims, question_embeds, q_conv_res_trigram, q_weights):
        ...
        insensitive_pooled  = self.pooling_method(sim_insens)
        sensitive_pooled    = self.pooling_method(sim_sens)
        oh_pooled           = self.pooling_method(oh_sims)
    ...
    def get_maxmin_max(self, res):
        ...
        res = torch.max(res)
        return res
    def emit_one(self, doc1_embeds, doc1_oh_sims, question_embeds, q_idfs, doc_gaf):
        ...
        doc1_oh_sims        = autograd.Variable(torch.FloatTensor(doc1_oh_sims),        requires_grad=False)
        doc_gaf             = autograd.Variable(torch.FloatTensor(doc_gaf),             requires_grad=False)
        # HANDLE QUESTION
        ...
        # HANDLE DOCS
        good_out            = self.emit_doc_cnn(doc1_embeds, doc1_oh_sims, question_embeds, q_context, q_weights)
        good_out_pp         = torch.cat([good_out, doc_gaf], -1)
        final_good_output   = self.final_layer(good_out_pp)
        return final_good_output
    def forward(self, doc1_embeds, doc2_embeds, doc1_oh_sims, doc2_oh_sims, question_embeds, q_idfs, doc_gaf, doc_baf):
        q_idfs              = autograd.Variable(torch.FloatTensor(q_idfs),              requires_grad=False)
        question_embeds     = autograd.Variable(torch.FloatTensor(question_embeds),     requires_grad=False)
        doc_gaf             = autograd.Variable(torch.FloatTensor(doc_gaf),             requires_grad=False)
        ...
        doc1_oh_sims        = autograd.Variable(torch.FloatTensor(doc1_oh_sims),        requires_grad=False)
        doc2_oh_sims        = autograd.Variable(torch.FloatTensor(doc2_oh_sims),        requires_grad=False)
        doc_gaf             = autograd.Variable(torch.FloatTensor(doc_gaf),             requires_grad=False)
        doc_baf             = autograd.Variable(torch.FloatTensor(doc_baf),             requires_grad=False)
        # HANDLE QUESTION
        q_context           = self.apply_context_convolution(question_embeds,   self.trigram_conv_1, self.trigram_conv_activation_1)
        ...
        # HANDLE DOCS
        good_out            = self.emit_doc_cnn(doc1_embeds, doc1_oh_sims, question_embeds, q_context, q_weights)
        bad_out             = self.emit_doc_cnn(doc2_embeds, doc2_oh_sims, question_embeds, q_context, q_weights)
        #
        good_out_pp         = torch.cat([good_out, doc_gaf], -1)
...
for run in range(0, 1):
    ...
    #
    odir                    = '/home/dpappas/DOC_CNN_PDRMM_BIOBERT_run_{}/'.format(run)
    #

...
def train_data_step2(instances, docs, wv, bioasq6_data, idf, max_idf, use_sent_tokenizer):
    for quest_text, quest_id, gid, bid, bm25s_gid, bm25s_bid in instances:
        #
        doc_toks = tokenize(docs[gid]['title'] + ' ' + docs[gid]['abstractText'])
        doc_tokens, good_doc_embeds = get_embeds(doc_toks, wv)
        #
        doc_toks = tokenize(docs[bid]['title'] + ' ' + docs[bid]['abstractText'])
        doc_tokens, bad_doc_embeds = get_embeds(doc_toks, wv)
        #
        quest_tokens, quest_embeds = get_embeds(tokenize(quest_text), wv)
        q_idfs = np.array([[idf_val(qw, idf, max_idf)] for qw in quest_tokens], 'float')
        #
...
for run in range(5):
    ...
    #
    odir = '/home/dpappas/DOC_CNN_PDRMM_nofeats_run_{}/'.format(run)
    #

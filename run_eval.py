...
def format_bioasq2treceval_qret(bioasq_data, system_name, filename):
    with open(filename, 'w') as f:
        for q in bioasq_data['questions']:
            rank = 1
            for d in q['documents']:
                sim = (len(q['documents']) + 1 - rank) / float(
                    len(q['documents']))  # Just a fake similarity. Does not affect the metrics we are using.
...
def trec_evaluate(qrels_file, qret_file):
    ...
    if VERSION == '8.1':
        ...
        trec_eval_res = subprocess.Popen(
            [os.path.dirname(os.path.realpath(__file__)) + '/trec_eval.9.0/./trec_eval', '-m', 'all_trec', qrels_file,
             qret_file],
            stdout=subprocess.PIPE, shell=False)

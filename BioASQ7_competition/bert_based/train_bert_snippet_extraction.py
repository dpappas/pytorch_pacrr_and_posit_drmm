...
def train_one(epoch, bioasq6_data, two_losses, use_sent_tokenizer):
    ...
    for datum in pbar:
        gs_emits_ = model(doc1_sents_embeds = datum['good_sents_embeds'], sents_gaf= datum['good_sents_escores'])
        ################################################################################
        ...
        cost_           = sn_d1_l
        ################################################################################
        emits           = [float(e > 0.5) for e in gs_emits_]
        batch_acc.extend(emits)
        epoch_acc.extend(emits)
        ################################################################################
...
def do_for_some_retrieved(docs, dato, retr_docs, data_for_revision, ret_data, use_sent_tokenizer):
    ...
    for retr in retr_docs:
        ...
        doc_emit_, gs_emits_    = model(doc1_sents_embeds=datum['sents_embeds'], sents_gaf=datum['sents_escores'])
...
class BERT_SNIP_Modeler(nn.Module):
    def __init__(self, embedding_dim=768):
        super(BERT_SNIP_Modeler, self).__init__()
    ...
    ##########################
    def forward(self, doc1_sents_embeds, sents_gaf):
...
for run in range(0, 1):
    ...
    ######
    odir = 'frozen_bioasq7_BERT_SNIPPET_2L_{}_run_{}/'.format(str(lr), run)
    ...
    model     = BERT_SNIP_Modeler(embedding_dim=embedding_dim).to(device)
    print_params(model)

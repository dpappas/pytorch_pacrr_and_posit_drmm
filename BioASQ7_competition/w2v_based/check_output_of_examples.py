...
def get_sents(some_text):
    ret = []
    for part in some_text.split('\n'):
        if(len(part)>0):
            ret.extend(sent_tokenize(part))
    return ret
...
def prep_data(quest, the_doc, the_bm25, wv, good_snips, idf, max_idf, use_sent_tokenizer):
    if(use_sent_tokenizer):
        good_sents  = get_sents(the_doc['title']) + get_sents(the_doc['abstractText'])
    else:
...
def do_for_some_retrieved(docs, dato, retr_docs, data_for_revision, ret_data, use_sent_tokenizer):
    ...
    for retr in retr_docs:
        ...
        to_return.append((doc_emit_, gs_emits_, datum['held_out_sents'], datum['sent_tags'], retr['doc_id']))
        # is_relevant, the_sent_score, ncbi_pmid_link, the_actual_sent_text
    ...
    return data_for_revision, ret_data, snips_res, snips_res_known, to_return
...
def get_one_map(prefix, data, docs, use_sent_tokenizer):
    ...
    for dato in tqdm(data['queries']):
        ...
        data_for_revision, ret_data, snips_res, snips_res_known, to_return = do_for_some_retrieved(docs, dato, dato['retrieved_documents'], data_for_revision, ret_data, use_sent_tokenizer)
        to_return2.extend(to_return)
...
snip_res    = []
vs          = softmax([item[0].item() for item in ret_data])
for item, doc_score in zip(ret_data, vs):
    for i in range(len(item[2])):
        sent_score  = item[1][i].item() * doc_score
        sent_text   = item[2][i]
        sent_label  = item[3][i]
        snip_res.append((sent_score, sent_text, sent_label))

snip_res    = sorted(snip_res, key=lambda x: x[0], reverse=True)
pprint(snip_res[:10])

for i in range(len(snip_res)):

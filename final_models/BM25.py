...
def similarity_score(query, document, k1, b, idf_scores, avgdl, normalize, mean, deviation, rare_word):
    ...
    for query_term in query:
        if query_term not in idf_scores:
            score += rare_word * ((tf(query_term, document) * (k1 + 1)) / (
    ...
    with open(dataset, 'r') as f1:
        ...
        if (True):
            for line in f1:
                ...
                ends.append(end)
                docs.append(did)
                BM25score = similarity_score(s1, s2, 1.2, 0.75, idf_scores, avgdl, normalized, mean, deviation,
                                             rare_word)
                bm25_scores.append(BM25score)
                if s1 in QA_pairs:
                    QA_pairs[s1].append((s2, label, BM25score))
                    Bio_pairs[(s1, qid, old_question)].append(
                        (s2, BM25score, old_question, old_answer, start, end, did))
                else:

...
class Sent_Posit_Drmm_Modeler(nn.Module):
    ...
    def forward(self, doc1_sents_embeds, doc2_sents_embeds, question_embeds, q_idfs, sents_gaf, sents_baf, doc_gaf, doc_baf):
        ...
        doc_emit_expanded               = final_bad_output.unsqueeze(-1).expand_as(bs_emits)
        bs_emits                        = torch.cat([bs_emits, doc_emit_expanded], -1)
        bs_emits                        = self.sent_out_combine_doc(bs_emits).squeeze(-1)
...
for run in range(0, 5):
    ...
    #
    odir    = 'super_model_3_2L_multihead_no_mesh_0p001_run_{}/'.format(run)

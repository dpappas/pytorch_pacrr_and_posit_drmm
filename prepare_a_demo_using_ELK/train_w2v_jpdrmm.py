...
def load_all_data(dataloc, w2v_bin_path, idf_pickle_path):
    ...
    #
    with open(dataloc+'traininng.json', 'r') as f:
        bioasq7_data_train  = json.load(f)
        bioasq7_data_train  = dict((q['id'], q) for q in bioasq7_data_train['questions'])
        f.close()
    #
    with open(dataloc+'dev.json', 'r') as f:
        bioasq7_data_dev    = json.load(f)
        bioasq7_data_dev    = dict((q['id'], q) for q in bioasq7_data_dev['questions'])
        f.close()
    #
    bioasq7_data = {}
    bioasq7_data.update(bioasq7_data_train)
    bioasq7_data.update(bioasq7_data_dev)
    #
    with open(dataloc+'bm25_top100.dev.json', 'r') as f:
        dev_data = json.load(f)
        f.close()
    #
    with open(dataloc+'bm25_top100.train.json', 'r') as f:
        train_data = json.load(f)
        f.close()
    #
    with open(dataloc+'bm25_docset.dev.json', 'r') as f:
        dev_docs = json.load(f)
        f.close()
    #
    with open(dataloc+'bm25_docset.train.json', 'r') as f:
